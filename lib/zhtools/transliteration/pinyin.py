...
class Pinyin(Syllable):
    ...
    def find_marked_vowel(syllable):
        ...
        Rules for tone mark placement can be found at
            http://www.pinyin.info/rules/where.html
        1. 'a' has priority over all
        2. 'e' comes right after
        3. 'o' receives the mark in ou (not in uo)
        4. in all other cases (which involve glides /w/, /ɥ/ and /j/),
           the last vowel takes the mark"""
        ...
        if "a" in syllable:
            ...
            regx = "(?P<last_vowel>[aeiouü])[^aeiouü]*$"
            return re.search(regx, syllable, re.IGNORECASE).group("last_vowel")
        ...
        if len(self.phones) > 0:
            if self.phones == "m":
                ...
                if 0 < self.tone < 5:
                    ...
                    replacement = v  # tone 5 is unmarked
                if not v.islower():
                    replacement = replacement.upper()
                self.syl = self.phones.replace(v, replacement)
